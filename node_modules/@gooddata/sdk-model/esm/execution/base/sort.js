...
export function sortEntityIds(sort) {
    ...
    if (isAttributeSort(sort)) {
        var attrId = sort.attributeSortItem.attributeIdentifier;
        res.attributeIdentifiers.push(attrId);
        res.allIdentifiers.push(attrId);
    }
    else if (isMeasureSort(sort)) {
        sort.measureSortItem.locators.forEach(function (loc) {
            if (isAttributeLocator(loc)) {
                var attrId = loc.attributeLocatorItem.attributeIdentifier;
                res.attributeIdentifiers.push(attrId);
                res.allIdentifiers.push(attrId);
            }
            else if (isMeasureLocator(loc)) {
                var measureId = loc.measureLocatorItem.measureIdentifier;
 ...
 * @public
 */
export function newAttributeAreaSort(attributeOrId, sortDirection, aggregation) {
    if (sortDirection === void 0) { sortDirection = "asc"; }
 ...
 * @public
 */
export function newMeasureSort(measureOrId, sortDirection, attributeLocators) {
    if (sortDirection === void 0) { sortDirection = "asc"; }
    if (attributeLocators === void 0) { attributeLocators = []; }
    invariant(measureOrId, "measure to create sort for must be specified");
    var id = measureLocalId(measureOrId);
    return {
        measureSortItem: {
            direction: sortDirection,
            locators: __spreadArrays(attributeLocators, [
                {
                    measureLocatorItem: {
 ...
 * @public
 */
export function newMeasureSortFromLocators(locators, sortDirection) {
    if (sortDirection === void 0) { sortDirection = "asc"; }
...
export function newAttributeLocator(attributeOrId, element) {
    ...
    var localId = attributeLocalId(attributeOrId);
    return {
