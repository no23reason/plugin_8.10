...
import { isBarChart, isBubbleChart, isHeatmap, isOneOfTypes, isScatterPlot, unwrap, isSupportingJoinedAttributeAxisName, } from "../_util/common";
...
export function getXAxes(dv, config, measureGroup, viewByAttribute, viewByParentAttribute) {
    var type = config.type, enableJoinedAttributeAxisName = config.enableJoinedAttributeAxisName;
    ...
    if (enableJoinedAttributeAxisName && isSupportingJoinedAttributeAxisName(type)) {
        var xLabel_1 = "";
        if (config.xLabel) {
            xLabel_1 = config.xLabel;
        }
        else if (viewByAttribute && viewByParentAttribute) {
            xLabel_1 = [viewByParentAttribute.formOf.name, viewByAttribute.formOf.name].join(" \u203a ");
        }
        else if (viewByAttribute) {
            xLabel_1 = viewByAttribute.formOf.name;
        }
        return [
            {
                label: xLabel_1,
